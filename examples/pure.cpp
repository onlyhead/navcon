...
int main() {
    auto rec = std::make_shared<rerun::RecordingStream>("ondrive_pure_pursuit_demo", "pure_pursuit");
    ...
    ondrive::Tracker navigator(ondrive::TrackerType::PURE_PURSUIT);
    ...
    for (int i = 0; i < 2000 && !navigator.is_path_completed(); ++i) {
        ...
        if (cmd.valid) {
            ...
            ondrive::visualize::show_robot_state(rec, robot_state, "robot_pure_pursuit", rerun::Color(0, 255, 0));
            navigator.tock();
