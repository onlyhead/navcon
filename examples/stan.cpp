...
int main() {
    auto rec = std::make_shared<rerun::RecordingStream>("ondrive_stanley_demo", "stanley");
    ...
    ondrive::Tracker navigator(ondrive::TrackerType::STANLEY);
    ...
    ondrive::PathGoal path_goal(build_s_shape_path(), 0.5f, 0.8f, false);
    ...
    for (int i = 0; i < 2000 && !navigator.is_path_completed(); ++i) {
        ...
        if (cmd.valid) {
            ...
            ondrive::visualize::show_robot_state(rec, robot_state, "robot_stanley", rerun::Color(255, 165, 0));
            navigator.tock();
