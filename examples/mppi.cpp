...
int main() {
    auto rec = std::make_shared<rerun::RecordingStream>("ondrive_mppi_demo", "mppi");
    ...
    ondrive::Tracker navigator(ondrive::TrackerType::MPPI);

    // Configure MPPI parameters
    auto mppi_controller = dynamic_cast<ondrive::pred::MPPIFollower *>(navigator.get_controller());
    if (mppi_controller) {
    ...
    for (; iteration < 2000 && !navigator.is_path_completed(); ++iteration) {
        ...
        if (cmd.valid) {
            ...
            // Visualize
            ondrive::visualize::show_robot_state(rec, robot_state, "robot_mppi",
